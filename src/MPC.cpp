 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < (int)N; t++) {
      ...
      fg[1 + psi_start + t] = psi1 - (psi0 +
                                      v0 * (t > 1 ? vars[delta_start + t - 2] :
                                            delta0) / Lf * dt);
      fg[1 + v_start + t] = v1 - (v0 + (t > 1 ? vars[a_start + t - 1] : a0) * dt);
      fg[1 + cte_start + t] =
          cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
      fg[1 + epsi_start + t] =
          epsi1 - ((psi0 - psides0) + v0 * (t > 1 ? vars[delta_start + t - 2] :
                                            delta0) / Lf * dt);
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  steering_angle_sol_ =  solution.x[delta_start];
  throttle_sol_ = solution.x[a_start];

  x_sol_.clear();
  y_sol_.clear();
  for (int i = 0; i < (int)N - 1; i++){
    x_sol_.emplace_back(solution.x[x_start + 1 + i]);
    y_sol_.emplace_back(solution.x[y_start + 1 + i]);
  }
